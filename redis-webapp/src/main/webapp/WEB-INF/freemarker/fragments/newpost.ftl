    ...
    <table><tr>
     ...
     <td style="vertical-align: top;">
      ...
      <span id="commentShow_${p.pid}" style="float:right;display:''">
        <a href="javascript:showComments('${p.pid}','${p.user.id}');">Comment(${p.commentNum})</a>&nbsp;
      </span>
      ...
      <span id="" style="float:right">
        <a href="javascript:collect('${p.pid}');">Collect</a>&nbsp;
      </span>
      ...
      <#--<a href="javascript:openWindow('<#if p.transmitid!='null' && p.webPost?? && p.webPost.content?has_content>${p.transmitid}<#else>${p.pid}</#if>');">Forward(${p.transmitNum})</a>-->
       <a href="javascript:openWindow('${p.pid}');">Forward(${p.transmitNum})</a>
      <#if manage??><a href="javascript:removeBlogByMe('${p.pid}');">Delete</a></#if>&nbsp;
      </span>
